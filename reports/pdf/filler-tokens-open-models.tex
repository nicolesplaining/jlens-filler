...
\begin{enumerate}
\item Patch the base's question-token residual into the chat model under an announced span, and the chat model's announced-span question token into its own unannounced run, to test whether deferral is carried by that one position. Then find the announcement's footprint: which heads read the ``there will be $N$ filler tokens'' sentence, and whether ablating them restores the chat model's in-place computation and its $k=0$ accuracy.

...
\begin{enumerate}
\item Find what in post-training created the $k=0$ deficit. Intermediate checkpoints from DeepSeek are not public, but the chat model's failure cases at $k=0$ can be characterized: which items it misses, what it emits instead, and whether the miss is a wrong binding or a wrong arithmetic step. Comparing the chat and base models' logit-lens trajectories at the answer position on the same items would show where the direct path diverges.

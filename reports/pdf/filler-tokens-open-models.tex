...
\section{Part VII: what fills the span, and when the model decides to use it}

A second day on the same box, with three questions. Does the filler have to be dots? Is there a point in the span where the model ``changes thought''? And what, in the context, makes the chat model stop computing at the question token? Everything here uses the same 50 released items for behavior and the same 50 held-out items at $k=50$ for dumps, in chat rendering unless stated (\ttt{reports/filler-types-and-cosine.md} is the running log).

\subsection{Letters, numbers, and their scrambles}

\ttt{src/jlens\_filler/prompts.py} renders five filler types: dots, the alphabet, a fixed scrambled alphabet, counting numbers, and a fixed scrambled sequence of numbers. Numbers cost two tokens each on DeepSeek's tokenizer (a standalone space precedes each), so 50 numbers are 99 tokens.

\begin{table}[h]\centering\scriptsize
\begin{tabular}{llrrrrrrll}\toprule
Filler & Model & $k$=0 & 5 & 10 & 25 & 50 & 100 & helped/hurt @50 & pre-q.\ @50 \\ \midrule
...
any other & base & 48 & 49--50 & 49--50 & 48--50 & 49--50 & 49--50 & $\leq$2/1 & --- \\ \bottomrule
\end{tabular}
\caption{Correct out of 50 by filler type and count (post-question), with the pre-question control at 50.}\end{table}
...
Model & Filler & gen$\to$filler & cos same pos & $R^2$ ans$|$filler & $R^2$ ans$|$q\_last & cent.\ adj. & correct @50 \\ \midrule
...
base & counting, scrambled & 0.267 & 0.72 & 0.88 & 0.76 & 0.00 & 50 \\ \bottomrule
\end{tabular}
\caption{Anatomy by filler type (50 held-out items, $k=50$ items). Attention is the generation position's mass on the filler over the last third of blocks.}\end{table}
...
The same fillers on Qwen3.5-9B: the untrained model is at floor for all of them, and the dots-only LoRA gets nothing from letters (42 $\to$ 40) and is hurt by numbers (42 $\to$ 37; scrambled, 42 $\to$ 30 with 12 items hurt and none helped). Its answer position, which ignores dots and letters (attention 0.01), attends to filler digits at 0.05 with single heads at 0.5: a model that does its arithmetic at the answer position reads filler digits as problem digits. DeepSeek's chat model has the opposite relation to the same tokens.

\subsection{Adjacent-position cosine: no change of thought}

For each item, the residual at every filler position and every block. Adjacent cosine is $\cos(h_p, h_{p+1})$ per block and flattened over blocks; the \emph{item-centered} version subtracts the per-position mean over items first, removing the position-identity component that dominates filler residuals. Change points are boundaries where an item's flattened centered series drops more than two standard deviations below its own mean (\ttt{scripts/analyze\_filler\_cosine.py}).

\begin{figure}[h]\centering\includegraphics[width=\textwidth]{deepseek_adjacent_cosine_flat.png}
\caption{Adjacent-position cosine across fifty dots, chat versus base: flattened over blocks (left), item-centered (middle), and the fraction of items with a change point at each boundary (right).}\end{figure}

\begin{table}[h]\centering\footnotesize
\begin{tabular}{lp{2.6cm}rp{4.2cm}}\toprule
...
\end{tabular}
\caption{Deferral is triggered by the announcement and graded by the announced count. Answer probes at the cue and generation positions stay at 0.79 to 0.83 in every condition.}\end{table}
...
\paragraph{Which channel carries it.} The announced-but-absent prompt announces in two places: the system sentence and the five demonstrations, each with fifty dots between question and answer. Separating them (\ttt{announce\_mode}) gives a double dissociation.

\begin{table}[h]\centering\small
\begin{tabular}{lrrrr}\toprule
Target $k=0$, announced by & chat: q\_last probe & chat: correct & base: q\_last probe & base: correct \\ \midrule
nothing & 0.84 & 34 & 0.84 & 48 \\
sentence only & 0.83 & 28 & 0.84 & 50 \\
demonstrations only & 0.40 & 34 & 0.55 & 46 \\
both & 0.31 & 32 & 0.55 & 46 \\ \bottomrule
\end{tabular}
\caption{The demonstrations move the computation off the question token; the sentence costs the chat model accuracy without moving it.}\end{table}

With fifty dots delivered in the target, the same split: nothing announced 34 $\to$ 39, sentence only 28 $\to$ 36, demonstrations only 34 $\to$ 47 (13 helped, 0 hurt), both 34 $\to$ 49. Unannounced dots help a little; demonstrations of a span being used make them work; the sentence adds two items on top.

So the deferral is in-context format learning: five examples of question, span, answer teach both checkpoints, the chat model more strongly, to finish the arithmetic after the question rather than at it, in proportion to the span the examples show (demonstrations alone, 5, 25, 50 dots: chat 0.49, 0.33, 0.40; base 0.84, 0.82, 0.55). The instruction sentence does something different. It costs the post-trained model six items (34 to 28, its worst score in any condition) without changing where the answer is computed, and it does nothing to the base.

\paragraph{Where the sentence is read.} Attributing attention keys to the 21-token filler sentence in the system message (a sixth region in \ttt{dump\_dot\_attention\_dsv4.py}): from the last question token, the chat model puts 0.04 of its attention on that sentence in blocks 20 to 24 (single heads at 0.16), 0.06 in blocks 26 to 38 (a head at 0.25), and 0.17 at block 40 (a head at 0.38), against a uniform-attention level of 0.026; the base stays at or below 0.009 through block 36 and reads it at 0.08 at block 40. The numbers are identical whether the dots are delivered or not. The sentence sits about 600 tokens before the question, outside the sliding window, so this is reading through the compressed keys in the even blocks, and its onset at block 20 is where the question-token probe first separates between announced and unannounced prompts. Post-training installed heads at the question token that read the filler instruction. Given the channel decomposition, they are not the deferral mechanism (the demonstrations do that); they are the chat model's route to a sentence whose presence costs it accuracy. What the demonstrations do to the question token is not located: under demonstrations-only announcement the question token's attention on the demonstrations' filler spans is at or below their share of the keys in both checkpoints (0.16 chat, 0.21 base, against about 0.32), so it is not a matter of reading those spans.
...
\begin{enumerate}
\item Patch the base's question-token residual into the chat model under an announced span, and the chat model's announced-span question token into its own unannounced run, to test whether deferral is carried by that one position. The announcement's footprint is now located (blocks 20 to 40 at the question token); the next step is to ablate those heads' attention to the sentence and see whether the chat model's in-place computation and its $k=0$ accuracy return.

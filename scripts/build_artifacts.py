...
def methods_present(result: dict[str, Any]) -> list[str]:
    """DeepSeek extractions carry j_lens and logit_lens; HF extractions carry logit_lens only."""
    return [m for m in ("j_lens", "logit_lens") if m in result["readouts"]]


def report_layers(result: dict[str, Any]) -> list[int]:
    available = sorted(map(int, result["readouts"][methods_present(result)[0]]))
    if set(REPORT_LAYERS) <= set(available):
        return REPORT_LAYERS
    n = len(available)
    picks = sorted({available[0], available[n // 4], available[n // 2], available[(3 * n) // 4]} | set(available[-7:]))
    return picks
...
def compact_viewer_data(result: dict[str, Any]) -> dict[str, Any]:
    ...
    for method in methods_present(result):
    ...
    return {
        ...
        "actualFinal": {
            "block": result.get("compatibility_checks", {}).get("n_blocks", 42),
            "label": f"L{result.get('compatibility_checks', {}).get('n_blocks', 42)}",
...
def rank_summary(result: dict[str, Any], method: str, target: str) -> dict[str, Any]:
    ...
    if not rows:
        return {"best": {"rank": None, "layer": None, "position": None, "top1": None}, "first_at_10": None}
...
def write_markdown_report(result: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        "",
        ("These are **J-Lens token readouts**, not the paper's formal sparse J-space decomposition." if "j_lens" in result["readouts"] else "These are **logit-lens token readouts** (final norm + unembedding applied to each block's residual); no Jacobian lens was used."),
        "",
        "## Outcome",
        "",
        ...
        f"`{result['compatibility_checks'].get('final_block_closure_max_abs_error', result['compatibility_checks'].get('layer_42_final_head_closure_max_abs_error'))}`.",
        "",
    ...
    for method, method_label in (("j_lens", "J-Lens"), ("logit_lens", "Logit lens")):
        if method not in result["readouts"]:
            continue
    ...
    primary = methods_present(result)[0]
    primary_label = "J-Lens" if primary == "j_lens" else "Logit lens"
    lines.extend(["", f"## {primary_label} top-5 by filler position", ""])
    layers = report_layers(result)
    for column_index, column in enumerate(result["selected_columns"]):
        ...
        for layer in layers:
            cell = result["readouts"][primary][str(layer)][column_index]
            target_ranks = ", ".join(
...
def build_viewer_html(data: dict[str, Any]) -> str:
    labels = {"j_lens": "J-Lens", "logit_lens": "Logit lens"}
    method_options = "".join(
        f'<option value="{method}">{labels[method]}</option>' for method in data["methods"]
    )
...
<div class="controls">
<label>Readout<select id="method">{method_options}</select></label>
...
function shownLayers(){{const all=Object.keys(DATA.methods[methodEl.value]).map(Number).sort((a,b)=>a-b);return layersEl.value==='all'?all:all.filter(x=>x%2===0||x===all[all.length-1])}}

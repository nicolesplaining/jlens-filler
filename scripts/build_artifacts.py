...
def write_markdown_report(result: dict[str, Any], path: Path) -> None:
    ...
    for method, method_label in (("j_lens", "J-Lens"), ("logit_lens", "Logit lens")):
        for target, surface in expected.items():
            ...
            lines.append(
                f"| {method_label} | `{surface}` | {best['rank']} "
    ...
    lines.extend(
        [
            "",
            "The clearest filler-localized signal is the tungsten value: J-Lens reads `74` "
            "as top-1 at layer 36/filler 4 and at several nearby late-layer cells. The numeric "
            "token `6` reaches rank 5 at layer 36/filler 8, where the top readout is the lexical "
            "token ` six`. The lexical sum form ` eighty` is top-1 at layer 40/filler 4, even "
            "though the numeric sum token `80` never reaches the filler-cell top 10; numeric `80` "
            "becomes rank 2 only at the layer-40 answer-prediction position before the final block.",
            "",
    ...
    for column_index, column in enumerate(result["selected_columns"][:10]):
        lines.extend(
            [
        ...
        for layer in layers:
            ...
            target_ranks = ", ".join(
                f"{expected[label]}={cell['targets'][label]['best_rank']}"
...
<style>
:root {{ color-scheme: light dark; --bg:#f7f8fb; --panel:#fff; --text:#172033; --muted:#657086; --line:#d8deea; --hot:#f59e0b; --j:#2563eb; --ll:#7c3aed; --t74:#0f9d74; --t6:#d97706; --t80:#dc2626; }}
@media (prefers-color-scheme:dark) {{ :root {{ --bg:#0d1220; --panel:#151c2d; --text:#edf2ff; --muted:#a8b2c7; --line:#344057; --hot:#fbbf24; --j:#60a5fa; --ll:#c4b5fd; --t74:#34d399; --t6:#fbbf24; --t80:#fb7185; }} }}
...
.scroll {{ overflow:auto; border:1px solid var(--line); background:var(--panel) }} .matrix {{ min-width:1060px; display:grid; grid-template-columns:68px repeat(12,minmax(78px,1fr)) }}
...
.cell {{ min-height:44px; padding:5px; border:0; border-right:1px solid var(--line); border-bottom:1px solid var(--line); background:transparent; color:var(--text); font:inherit; cursor:pointer; overflow-wrap:anywhere }} .cell:hover,.cell:focus {{ outline:2px solid var(--j); outline-offset:-2px }} .cell.hit74 {{ background:color-mix(in srgb,var(--t74) 20%,transparent) }} .cell.hit6 {{ background:color-mix(in srgb,var(--t6) 20%,transparent) }} .cell.hit80 {{ background:color-mix(in srgb,var(--t80) 20%,transparent) }} .cell.unavailable {{ cursor:default; color:var(--muted); border-top:3px solid var(--line) }} .cell.actual {{ border-top:3px solid var(--t80); background:color-mix(in srgb,var(--t80) 24%,transparent); font-weight:600 }} .layer.actual-layer {{ border-top:3px solid var(--t80); color:var(--t80) }}
...
.heat {{ overflow:auto; border:1px solid var(--line); background:var(--panel) }} .heat-grid {{ min-width:760px; display:grid; grid-template-columns:54px repeat(10,minmax(54px,1fr)) }} .heat-label,.heat-head {{ padding:3px 5px; color:var(--muted); border-bottom:1px solid var(--line) }} .heat-cell {{ min-height:22px; border:0; border-right:1px solid var(--line); border-bottom:1px solid var(--line); color:var(--text); font:12px system-ui; cursor:pointer }}
...
<div class="legend"><span><i class="dot" style="background:var(--t74)"></i>`74` rank ≤10 or highlighted surface form</span><span><i class="dot" style="background:var(--t6)"></i>`6` rank ≤10 or highlighted surface form</span><span><i class="dot" style="background:var(--t80)"></i>`80` signal / actual model row</span></div>
...
<script>
const DATA={payload};
const methodEl=document.getElementById('method'), layersEl=document.getElementById('layers'), matrixEl=document.getElementById('matrix'), detailEl=document.getElementById('detail'), targetsEl=document.getElementById('targets');
const labels=Object.keys(DATA.expected), colors=['var(--t74)','var(--t6)','var(--t80)'];
...
function hitClass(cell){{const top=cell.top[0][0].trim().toLowerCase();for(let i=0;i<labels.length;i++){{const r=cell.targets[labels[i]].rank,forms=(DATA.highlightForms[labels[i]]||[]).map(x=>x.trim().toLowerCase());if((r!==null&&r<=10)||forms.includes(top))return ['hit74','hit6','hit80'][i]}}return ''}}

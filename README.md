...
- Raw residual/attention dumps (`dot_dump.pt`, `dot_attention.pt`, 60 files, 86 GB) for every DeepSeek, Qwen, Llama and Gemma condition are archived in the private HF dataset `nick-rui/jlens-filler-dumps` (same directory layout as `results/`).
- Filler types (letters, numbers, scrambled) on DeepSeek chat and base, adjacent-position cosine along the span, k=0 and announced-but-absent dumps: `reports/filler-types-and-cosine.md` (PDF Part VII). Numbers work like dots, letters less, any order; no change-of-thought point in the span; announcing a filler span moves the chat model's computation off the question token in proportion to the promised count (0.84 -> 0.51/0.36/0.31 for 5/25/50 announced, none delivered). Scripts `run_dsv4_filler_types.sh`, `run_dsv4_followups.sh`, `run_dsv4_announce.sh`, `analyze_filler_cosine.py`, `summarize_filler_types.py`, `probe_k0_dump.py`.
